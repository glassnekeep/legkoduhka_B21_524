...
import librosa # поставлена вручную, а не через менеджер пакетов пайчарма
...
def main():
    ...
    files = ['letter_a', 'letter_i', 'gav']
    humiliation_voice_paths = [
        (humiliation, os.path.join(input_path, f'{humiliation}.wav'))
        for humiliation in files
    ]
    with open(os.path.join(output_path, 'res.txt'), 'w') as res_file:
        for file, voice_path in humiliation_voice_paths:
            rate, samples = wavfile.read(voice_path)
            freq, t, spec = spectrogram(samples, rate, os.path.join(output_path, f'{file}.png'))
            max_freq, min_freq = get_max_min(voice_path)

            res_file.write(f'{file}:\n')
            res_file.write(f'\tМакс частота: {max_freq}\n')
            res_file.write(f'\tМин частота: {min_freq}\n')
